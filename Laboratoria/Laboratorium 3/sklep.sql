...
DROP VIEW IF EXISTS kwerenda_1;
CREATE VIEW kwerenda_1 AS
SELECT imie, nazwisko
FROM klient
WHERE miasto = "Legionowo" OR miasto = "Warszawa";

SELECT * FROM kwerenda_1;

DROP VIEW IF EXISTS kwerenda_2;
CREATE VIEW kwerenda_2 AS
SELECT nazwa
FROM produkty
WHERE cena_brutto > 500;

SELECT * FROM kwerenda_2;

DROP VIEW IF EXISTS kwerenda_3;
CREATE VIEW kwerenda_3 AS
SELECT nazwa_kategorii, AVG(cena_brutto) AS "srednia cena"
FROM kategorie 
JOIN produkty ON produkty.id_kat = kategorie.id_kat
GROUP BY nazwa_kategorii;

SELECT * FROM kwerenda_3;

DROP VIEW IF EXISTS kwerenda_4;
CREATE VIEW kwerenda_4 AS
SELECT id_zam, ilosc, wartosc, nazwa 
FROM zamowienia 
JOIN produkty ON produkty.id_prod = zamowienia.id_prod
WHERE id_klient = 3;

SELECT * FROM kwerenda_4;
COMMIT;
