...
--
-- Host: 127.0.0.1
-- Czas generowania: 15 Lut 2024, 12:10
...
-- Procedury
--
CREATE PROCEDURE `first` ()   BEGIN
SELECT MONTH(`data`) AS miesiac, COUNT(`id`) AS ilosc
FROM koncerty
GROUP BY miesiac
HAVING miesiac=7;
END$$

CREATE PROCEDURE `second` ()   BEGIN
SELECT Miasta.kod_miasta, Sum(zad_2.liczba_artystow) AS SumaOfliczba_artystow
FROM Miasta INNER JOIN zad_2 ON Miasta.kod_miasta = zad_2.kod_miasta
GROUP BY Miasta.kod_miasta
ORDER BY Sum(zad_2.liczba_artystow) DESC
LIMIT 2;
END$$

CREATE PROCEDURE `third` ()   BEGIN
SELECT Miasta.wojewodztwo, AVG(zad_3.PoliczOfid) AS srednia
FROM Miasta INNER JOIN zad_3 ON Miasta.kod_miasta = zad_3.kod_miasta
GROUP BY Miasta.wojewodztwo
ORDER BY srednia DESC;
END$$

CREATE PROCEDURE `fourth`()
BEGIN
SELECT Zespoly.nazwa
FROM Zespoly
WHERE (Zespoly.nazwa) Not In (SELECT nazwa FROM  zad_4);
END$$

CREATE PROCEDURE `fifth` ()   BEGIN
SELECT zad_5_1.nazwa, zad_5_1.weekend, zad_5_2.dni
FROM zad_5_2 INNER JOIN zad_5_1 ON zad_5_2.nazwa = zad_5_1.nazwa
WHERE ((zad_5_1.weekend)>dni);
END$$
...
--
-- Zastąpiona struktura widoku `zad_2`
...
--
-- Zastąpiona struktura widoku `zad_3`
-- (Zobacz poniżej rzeczywisty widok)
--
CREATE TABLE `zad_3` (
`wojewodztwo` varchar(19)
,`kod_miasta` varchar(6)
,`PoliczOfid` bigint(21)
);

-- --------------------------------------------------------

--
-- Zastąpiona struktura widoku `zad_4`
-- (Zobacz poniżej rzeczywisty widok)
--
CREATE TABLE `zad_4` (
`nazwa` varchar(21)
...
--
-- Zastąpiona struktura widoku `zad_5_1`
-- (Zobacz poniżej rzeczywisty widok)
--
CREATE TABLE `zad_5_1` (
`nazwa` varchar(21)
,`weekend` bigint(21)
);
...
--
-- Zastąpiona struktura widoku `zad_5_2`
-- (Zobacz poniżej rzeczywisty widok)
--
CREATE TABLE `zad_5_2` (
`nazwa` varchar(21)
,`dni` bigint(21)
);
...
--
-- Struktura widoku `zad_2`
--
DROP TABLE IF EXISTS `zad_2`;

CREATE VIEW `zad_2`  AS SELECT `miasta`.`kod_miasta` AS `kod_miasta`, `zespoly`.`id_zespolu` AS `id_zespolu`, `zespoly`.`liczba_artystow` AS `liczba_artystow` FROM ((`zespoly` join `koncerty` on(`zespoly`.`id_zespolu` = `koncerty`.`id_zespolu`)) join `miasta` on(`koncerty`.`kod_miasta` = `miasta`.`kod_miasta`)) GROUP BY `miasta`.`kod_miasta`, `zespoly`.`id_zespolu`, `zespoly`.`liczba_artystow`;

-- --------------------------------------------------------

--
-- Struktura widoku `zad_3`
--
DROP TABLE IF EXISTS `zad_3`;

CREATE VIEW `zad_3`  AS SELECT `miasta`.`wojewodztwo` AS `wojewodztwo`, `miasta`.`kod_miasta` AS `kod_miasta`, count(`id`) AS `PoliczOfid` FROM (`miasta` join `koncerty` on(`miasta`.`kod_miasta` = `koncerty`.`kod_miasta`)) GROUP BY `miasta`.`wojewodztwo`, `miasta`.`kod_miasta`;

-- --------------------------------------------------------

--
-- Struktura widoku `zad_4`
--
DROP TABLE IF EXISTS `zad_4`;

CREATE VIEW `zad_4`  AS SELECT `zespoly`.`nazwa` AS `nazwa` FROM (`zespoly` join `koncerty` on(`zespoly`.`id_zespolu` = `koncerty`.`id_zespolu`)) WHERE `data` <= '2017-7-25' AND `data` >= '2017-7-20' GROUP BY `zespoly`.`nazwa`;

-- --------------------------------------------------------

--
-- Struktura widoku `zad_5_1`
--
DROP TABLE IF EXISTS `zad_5_1`;

CREATE VIEW `zad_5_1`  AS SELECT `zespoly`.`nazwa` AS `nazwa`, count(`id`) AS `weekend` FROM (`zespoly` join `koncerty` on(`zespoly`.`id_zespolu` = `koncerty`.`id_zespolu`)) WHERE weekday(`data`) = 0 OR weekday(`data`) = 6 GROUP BY `zespoly`.`nazwa`;

-- --------------------------------------------------------

--
-- Struktura widoku `zad_5_2`
--
DROP TABLE IF EXISTS `zad_5_2`;

CREATE VIEW `zad_5_2`  AS SELECT `zespoly`.`nazwa` AS `nazwa`, count(`id`) AS `dni` FROM (`zespoly` join `koncerty` on(`zespoly`.`id_zespolu` = `koncerty`.`id_zespolu`)) WHERE weekday(`data`) = 1 OR weekday(`data`) = 2 OR weekday(`data`) = 3 OR weekday(`data`) = 4 OR weekday(`data`) = 5 GROUP BY `zespoly`.`nazwa`;
...
--
ALTER TABLE `koncerty`
  ADD CONSTRAINT `koncerty_ibfk_1` FOREIGN KEY (`id_zespolu`) REFERENCES `zespoly` (`id_zespolu`),
  ADD CONSTRAINT `koncerty_ibfk_2` FOREIGN KEY (`kod_miasta`) REFERENCES `miasta` (`kod_miasta`);
COMMIT;
